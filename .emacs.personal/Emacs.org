...
- [[#package-system-setup][Package System Setup]]
  ...
  - [[#ignore-pesky-warnings-of-functions-not-found][Ignore pesky warnings of functions not found]]
  - [[#keep-folders-clean][Keep folders clean]]
- [[#configure-projectel-to-avoid-conflicts][Configure Project.el to avoid conflicts]]
...
- [[#which-key][Which-key]]
- [[#evil-mode][Evil-mode]]
  - [[#default-to-evil-normal-mode-when-switching-buffer][Default to evil normal mode when switching buffer]]
- [[#magit][Magit]]
- [[#org-mode][Org Mode]]
...
- [[#python-configuration][Python configuration]]
  - [[#use-treesit-auto-for-automatic-detection-of-languages][Use treesit-auto for automatic detection of languages]]
  - [[#treesitter-for-python-formatting][Treesitter for python formatting]]
  - [[#code-folding-using-the-modern-python-ts-mode][Code folding using the modern python-ts-mode]]
  - [[#indent-bars][indent-bars]]
  - [[#eglot][eglot]]
  - [[#use-ruff-formatter-for-lintingformatting][Use ruff-formatter for linting/formatting]]
  - [[#use-uv-for-virtual-environment-management][Use uv for virtual environment management]]
  - [[#tell-tramp-to-use-the-remote-path-if-using-tramp][Tell TRAMP to use the remote PATH (if using TRAMP)]]
- [[#yaml-mode][YAML mode]]
- [[#eglot-old-version---to-remove][Eglot (old version - to remove)]]
- [[#vterm][vterm]]
...
- [[#copilot][Copilot]]
- [[#sly-lisp][Sly LISP]]
  - [[#use-roswell][Use Roswell]]
- [[#ess---r][ESS - R]]
- [[#keycasting][Keycasting]]
  - [[#use-keycast-to-see-it-in-the-header][Use keycast to see it in the header]]
  - [[#use-command-log-mode-to-see-it-on-the-side][Use command-log-mode to see it on the side]]
...
#+end_src
** Ignore pesky warnings of functions not found
#+begin_src emacs-lisp
...
#+end_src
* Configure Project.el to avoid conflicts
#+begin_src emacs-lisp
  (use-package project
    :ensure t)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (when (eq system-type 'darwin)
    (setq es/default-font-size 200
	  es/default-variable-font-size 200))
...
Emacs default keybindings for cursor movement are hard to use. Switching to evil, let's see if they are better
...
#+begin_src emacs-lisp
  ...
  (use-package evil-collection
    :after evil
    :init
    ;; (setq evil-collection-key-blacklist '("C-.")) ;; not working
    :config
...
** Default to evil normal mode when switching buffer

Thanks to Gemini.
NOTE It's messing with coding when there is vterm open?

# #+begin_src emacs-lisp
#   (defun my-evil-normal-state-on-buffer-switch (&rest _)
#     "Force evil normal state when switching buffers, except in the minibuffer."
#     (when (and (bound-and-true-p evil-mode)
#                (not (minibufferp)))
#       (evil-normal-state)))

#   ;; Hook into window configuration changes (covers buffer switching, pane splits, etc.)
#   (add-hook 'window-configuration-change-hook #'my-evil-normal-state-on-buffer-switch)

#   ;; Hook into buffer list changes (covers switching buffers within the same window)
#   (add-hook 'buffer-list-update-hook #'my-evil-normal-state-on-buffer-switch)
# #+end_src
...
#+begin_src emacs-lisp
  (setq org-base-path (expand-file-name "~/Nextcloud-stallone/Documents/org"))
  ...
  (use-package org
    ...
    (setq org-directory org-base-path
    	...
    	org-agenda-files '("Tasks.org" "piano-log.org" "piano.org" "work-log.org"
    			   "Archive.org" "chess-analysis.org" "mybooks.org" "garden.org")
  	 ...
  	 ((alltodo ""
  		   ((org-agenda-prefix-format "   %t   %s")
  		    ...
  		    (org-super-agenda-groups '((:name "Active" :todo "ACTIVE" :order 0)
  					       ...
  					       (:name "Garden" :tag "garden" :order 8)  					       
  					       ))))))
  	("w" "This Week"
  	 ((tags-todo "ThisWeek"
...
NOTE I might not need this section
...
#+begin_src emacs-lisp
  ...
  (use-package embark
    :ensure t
    :bind
    (("C-c k" . embark-act)         ;; pick some comfortable binding
...
First install basedpyright and ruff
#+begin_src bash
  uv tool install basedpyright
  uv tool install ruff
#+end_src

** Use treesit-auto for automatic detection of languages
...
** Treesitter for python formatting
...
#+begin_src emacs-lisp
  (use-package treesit
    ...
    ;; Automatically download python grammar if it is missing
    (unless (treesit-language-available-p 'python)
      (treesit-install-language-grammar 'python)))
#+end_src

** Code folding using the modern python-ts-mode

NOTE =treesit-fold= is for the newer emacs with built-in treesitter.
=ts-fold= is for old emacs relying on third party treesitter.

#+begin_src emacs-lisp
  (use-package treesit-fold
    :straight (treesit-fold :type git :host github :repo "emacs-tree-sitter/treesit-fold")
    :commands (treesit-fold-mode treesit-fold-indicators-mode)
    :init
    (add-hook 'python-ts-mode-hook #'treesit-fold-mode)
    (add-hook 'python-ts-mode-hook #'treesit-fold-indicators-mode)
    :config
    (setq treesit-fold-indicators-fringe 'left-fringe
  	treesit-fold-indicators-open-indicator 'right-triangle))
#+end_src

NOTE These are the Evil mode basic commands to fold:
...
#+begin_src emacs-lisp
  (use-package eglot
    :straight nil
    :hook
    ;; Automatically start Eglot when opening python files
    (python-ts-mode . eglot-ensure)
    :config
    ;; use basedpyright as language server
    ;; (add-to-list 'eglot-server-programs
    ;; 	       '((python-ts-mode python-mode) . ("basedpyright-langserver" "--stdio")))
    (add-to-list 'eglot-server-programs
  	       '((python-ts-mode python-mode) . ("uvx" "--from" "basedpyright" "basedpyright-langserver" "--stdio")))
...
** Use ruff-formatter for linting/formatting
Again, thanks to Gemini, I found this package to run =ruff= as a linter and formatter.
I do this because eglot only supports one lsp server at a time, which is currently occupied by =basedpyright= for type checking.
ruff-formatter allows you to call the CLI version of ruff as standalone. There is also an option to format on save, but I don't set it as default.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package uv-mode
    ...
    :config
    ;; Optional: If you switch environments or run 'uv sync' while a file is open,
    ;; this hook ensures Eglot automatically restarts to pick up the changes.
    (add-hook 'uv-mode-post-activate-hook #'eglot-reconnect))
#+end_src

** Tell TRAMP to use the remote PATH (if using TRAMP)
#+begin_src emacs-lisp
  (use-package tramp
    :straight nil
    :config
...
Without this fix, I can't write anything, it keeps trying to use ispell
to complete words and fails to find it.
...
* ESS - R
This is to run R and other stats packages
...
#+end_src
* Keycasting
** Use keycast to see it in the header
I wanted to see it in the mode-line, but it is not allowed by doom-modeline that disables standard regions.
I could overwrite it, but it is easier done in the header.
#+begin_src emacs-lisp
  (use-package keycast
    :ensure t
    :config
...
#+end_src
** Use command-log-mode to see it on the side
This is not necessary, only if I ever want to make a presentation.
NOTE not sure it is working.
