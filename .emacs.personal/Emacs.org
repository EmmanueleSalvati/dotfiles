...
- [[#tree-sit-auto-to-remove][Tree-sit auto (to remove)]]
- [[#treesit-fold-to-remove][Treesit-fold (to remove)]]
- [[#python-mode-to-remove][Python mode (to remove)]]
  - [[#pyvenv-to-remove][pyvenv (to remove)]]
...
- [[#yaml-mode][YAML mode]]
- [[#uv-mode-on-python-to-remove][uv mode on python (to remove)]]
...
* uv mode on python (to remove)
This is the automatically run uv in a project that has a .venv directory in it.
I just do not know if I should still use =python-ts-mode=, or =eglot=.
TODO: this needs to be fixed. as well as the eglot section.
